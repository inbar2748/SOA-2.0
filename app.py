...
######################################################################################################################################################################################    
    ...
    # You can adapt this code according to your actual data structure.
...
#       plt.plot(XY[:, 0], f, '-y', linewidth=3)
    ...
    print("--------------------------- AFTER MEASURED BRANCH ANALYSIS, NOW BELOW IS THE INFORMATION FOR SIMULATED RANDOM LINES----------------------------------------")    
...
#    BINOMIAL_LINES=create_binomial_lines(n,p,num_lines)
    ...
    plt.xlabel('Parallel Groups with Branches')
    plt.ylabel('Frequency Count for respective groups')
    plt.title("RANDOM-Total branches== "+str(sum(gbcountRANDOM))+ ", Arranged into"+str(len(gbcountRANDOM))+" groups")
    plt.show()      
...
######################################################################################################################################################################################    
    ...
    result_df = pd.concat([Logs_df, BL_df, ANGLE_df,BP_df,BranchesCompound_df,BranchesDetached_df,BranchesRetained_df,FileName_df], axis=1, ignore_index=True)
...
# Create buttons
load_button = tk.Button(bottom_frame, text="(1) Load Image", font=("Arial", 16), command=load_image, bg="blue", fg="white")
load_button.pack(side=tk.LEFT)

# Create sliders
slider1_var = tk.DoubleVar()
slider1_label = tk.Label(bottom_frame, text="<=THRESH",font=("Arial", 16))
slider1 = tk.Scale(bottom_frame, variable=slider1_var, from_=0, to=10, orient="horizontal", command=lambda _: update_slider_value1(slider1_var, slider1_label))
...
slider2 = tk.Scale(bottom_frame, variable=slider2_var, from_=0, to=10, orient="horizontal", command=lambda _: update_slider_value2(slider2_var, slider2_label))
...
slider3 = tk.Scale(bottom_frame, variable=slider3_var, from_=0, to=10, orient="horizontal", command=lambda _: update_slider_value3(slider3_var, slider3_label))
slider3.pack(side=tk.LEFT)
slider3_label.pack(side=tk.LEFT)

# Create buttons
segmentation_button = tk.Button(bottom_frame, text="(2) Create Segmentation", font=("Arial", 16), command=create_segmentation2, bg="blue", fg="white")
segmentation_button.pack(side=tk.LEFT)

# Create buttons
results_button = tk.Button(bottom_frame, text="(3) Plot Dendrites", font=("Arial", 16), command=generate_results, bg="blue", fg="white")
results_button.pack(side=tk.LEFT)


# Create buttons
export_button = tk.Button(bottom_frame, text="(4) Save Statistics", font=("Arial", 16), command=write_results, bg="red", fg="white")
